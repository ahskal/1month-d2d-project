...
void Dwrite::PosRenderText(wstring text, Vector2 Pos, float size)
{
	RECT rect;
	rect.left = Pos.x;
	rect.top = Pos.y;
	...
	RenderText(text, rect, size, font, color);
}

void Dwrite::PosRenderText(wstring text, Vector2 Pos, Color color, float size)
{
	RECT rect;
	rect.left = Pos.x;
	rect.top = Pos.y;
