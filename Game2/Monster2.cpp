...
Monster2::Monster2(wstring file, Int2 maxFream, wstring file2, Int2 maxFream2,
	float length, int Hp, int attackSize, float aniSpeed, int atk, int dropExp)
{
	AniSpeed = aniSpeed;
	Atk = atk;
	AttackSize = Vector2(attackSize);
...
void Monster2::Move(Vector2 TargetPos)
{
...
void Monster2::Attack()
{
	if (M_State == State::ATTACK && is_atk) {
		...
		if (attack->frame.x + 1 == attack->maxFrame.x && TIMER->GetTick(timer, AniSpeed)) {
			Bullet* B = new Bullet();
			Vector2 dir = Vector2(AttackPos - col->GetWorldPos());
			dir.Normalize();
			B->scale = AttackSize;
			B->Fire(col->GetWorldPos(), dir, AttackSize, 100, 10);
